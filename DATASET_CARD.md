...
2. Number the stations 0 to 12 in spatial order, sorting by the y coordinate and breaking ties by x, so a station number never reveals when it was visited.
...
5. Draw each leg as an anti-aliased straight line of half-width 0.9 pixels, then erase one contiguous interior span covering up to 94 percent of the leg. Both endpoints stay attached.
...
- **Source** - original synthetic data, generated by the creator; no records were downloaded from anywhere
- **Source repository** - <https://github.com/ha12401/traverse-order-recovery-dataset>
- **Generator** - `generate_raw.py` in that repository; running it reproduces `scenes.npz` byte for byte
- **Integrity** - `generation.json` in that repository records the SHA-256 of `scenes.npz`
- **License** - Creative Commons CC0 1.0 Universal, <https://creativecommons.org/publicdomain/zero/1.0/>
- **Attribution** - none required; no external material is incorporated, so no upstream licence or redistribution restriction applies
